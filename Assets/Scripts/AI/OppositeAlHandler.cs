...
public class OppositeAIHandler : MonoBehaviour
{
    [SerializeField]
    ...
    [SerializeField]
    float lane1Center = -14.1f;  // Левая встречная полоса

    [SerializeField]
    float lane2Center = -12.87f; // Правая встречная полоса
    ...
    void Start()
    {
        ...
        if (oppositeCarPrefabs.Length > 0)
        {
            ...
            for (int i = 0; i < oppositeCarPool.Length; i++)
            {
                ...
                // Удаляем старый AIHandler если есть
                AIHandler oldAi = oppositeCarPool[i].GetComponent<AIHandler>();
                if (oldAi != null)
                    Destroy(oldAi);

                // Добавляем OppositeAIHandler
    ...
    {
        // Случайно выбираем левую или правую встречную полосу
    ...
    {
        // Визуализация полос встречного движения
